      ...
      <Stack gap="default">
        ...
        <Stack direction="row" gap="sm">
          {WITHDRAW_QUICK_PICKS.map((pick) => (
            <Button key={pick.label} variant="secondary" size="sm" onPress={() => setValue(rawDigitsFromAmount(MOCK_BALANCE * pick.pct))}>{pick.label}</Button>
          ))}
          <Button variant="secondary" size="sm" onPress={() => setValue('')}>Outro</Button>
        </Stack>
        <DataList data={[
